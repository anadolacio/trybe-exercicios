const books = [
  ...
  function nameAndAge() {
    const buildingArray = books.map((book) => {
        const buildingNewArray = {};
        buildingNewArray.author = `${book.author.name}`;
        buildingNewArray.age = `${(book.releaseYear) - (book.author.birthYear)}`;
        return buildingNewArray;
    });

       return buildingArray.sort((a, b) => a.age - b.age);
